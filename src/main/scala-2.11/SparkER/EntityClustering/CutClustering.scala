...
object CutClustering extends EntityClusteringTrait {
  ...
  override def getClusters(profiles: RDD[Profile], edges: RDD[WeightedEdge], maxProfileID: Int, edgesThreshold: Double,
                           separatorID: Long): RDD[(Long, Set[Long])] = {
    getClusters(profiles, edges, maxProfileID, edgesThreshold, separatorID, 0.3)
  }

  def getClusters(profiles: RDD[Profile], edges: RDD[WeightedEdge], maxProfileID: Int, edgesThreshold: Double,
                  separatorID: Long, acap: Double): RDD[(Long, Set[Long])] = {
    ...
    EntityClusterUtils.addUnclusteredProfiles(profiles, res.flatMap(x => x).zipWithIndex().map(_.swap))
  }
}
