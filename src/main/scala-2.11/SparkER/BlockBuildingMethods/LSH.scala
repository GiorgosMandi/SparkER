...
object LSH {
  ...
  def createBlocks(profiles: RDD[Profile], numHashes: Int, targetThreshold: Double, numBands: Int = -1, separatorIDs: Array[Long] = Array.emptyLongArray, keysToExclude: Iterable[String] = Nil): RDD[BlockAbstract] = {
    @transient lazy val log = org.apache.log4j.LogManager.getRootLogger

    val t0 = Calendar.getInstance()

    /* Generate the tokens */
    val profilesToken: RDD[(Long, String)] = profiles.flatMap {
      profile =>
        val attributes = profile.attributes.filter(kv => !keysToExclude.exists(_.equals(kv.key)))
        attributes.flatMap {
          kv =>
    ...
    val profilesWithSignature: RDD[(Long, Array[Int])] = tokensPerAttribute.map { case (profileID, tokens) =>
